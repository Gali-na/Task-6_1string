...
public class FindNumbers {
    ...
    public int[] getAllNumbers(String text) {
        ...
        String[] tempStringArry = temp.split(" ");
        ArrayList<Integer> rezult = new ArrayList<>();
        for (int i = 0; i < tempStringArry.length; i++) {
            if (!tempStringArry[i].isEmpty()) {
                rezult.add(Integer.parseInt(tempStringArry[i]) * 2);
            }
        }
        int[] dicision = new int[rezult.size()];
        int j = 0;
        for (Integer namber : rezult) {
            dicision[j] = namber;
            j++;
        }
        return dicision;
